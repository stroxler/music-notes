# This file is adapted from `abc/__init__.py` in the music21 library (see
# the copyright notice below) - I've hacked it up in an attempt to explore
# my own extended notation that uses the letters H - L to replace accidentals,
# which required not only changing the note regexp but also disabling some
# accent-related logic that uses those letters in traditional abc.
#
# I don't expect this to be useful long-term, and of course the class names
# make no sense right now since I left them unchanged, but this allows me
# to explore the idea quickly without building a new parser from scratch.
...
rePitchName = re.compile('[a-lA-Lz]')
    ...
    def tokenize(self, strSrc: str) -> None:
        ...
        while self.pos < self.srcLen - 1:
            ...
            if c.isalpha() or c in '~^=_':
                ...
                if self.currentCollectStr in ('w', 'u', 'v', 'v.', 'vk',
                               'uk', 'U', '~',
                               '.', '=', 'V', 'v.', 'S', 's',
                               # DISABLED FOR HIJ
                               # 'h', 'H', 'i', 'I', l', 'L',
